...
PolynomialInterpolation::PolynomialInterpolation(uint16_t maxPoint)
{
...
void PolynomialInterpolation::getPoint(uint16_t num, float *value, float *adc)
{
	if(num < mMaxPoint)
	{
		*value = mValue[num];
...
void PolynomialInterpolation::setNumberOfPoint(uint16_t num)
{
	if(num < 2)
		mWorkingPoint = mMaxPoint;
...
uint16_t PolynomialInterpolation::getNumberOfPoint(void)
{
	return mWorkingPoint;
}

float PolynomialInterpolation::calculate(float adc)
{
	float buf = 0;
	int32_t  index;
